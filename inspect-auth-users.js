...
(async () => {
  console.log('Testing public query on companies...');
  const { data: comp, error: compErr } = await supabase.from('companies').select('*');
  console.log('Companies:', comp, compErr?.message);
})();
