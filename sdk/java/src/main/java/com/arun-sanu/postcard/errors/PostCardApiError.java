 ...
 * subclass.
 */
public class LeadWeaveApiError extends LeadWeaveError {
    ...
    public LeadWeaveApiError(String message, int status, Object body, String errorKind) {
        super(message);
        this.status = status;
        this.body = body;
     ...
     * surfaces as a generic {@link LeadWeaveApiError} whose message states the
     * redirect was not followed (the API key is never re-sent to a redirect
     * target).
     */
    public static LeadWeaveApiError fromResponse(int status, String statusText, String rawBody, String context) {
        ...
        String message = "LeadWeave API " + status + reason + " — " + context + tail;
    ...
    public static LeadWeaveApiError classify(int status, String message, Object body, String errorKind) {
        return switch (status) {
            case 401 -> new LeadWeaveAuthError(message, status, body, errorKind);
            case 403 -> new LeadWeaveForbiddenError(message, status, body, errorKind);
            case 404 -> new LeadWeaveNotFoundError(message, status, body, errorKind);
            case 409 -> new LeadWeaveConflictError(message, status, body, errorKind);
            case 429 -> new LeadWeaveRateLimitError(message, status, body, errorKind);
            case 501 -> new LeadWeaveNotImplementedError(message, status, body, errorKind);
            case 503 -> new LeadWeaveServiceUnavailableError(message, status, body, errorKind);
            default -> new LeadWeaveApiError(message, status, body, errorKind);
        };
    }
}
