/**
 * LeadWeave JavaScript/TypeScript SDK — client core.
 *
 * The {@link LeadWeaveClient} is the single entry point. It holds configuration
...
export interface LeadWeaveClientOptions {
...
export class LeadWeaveClient {
  private readonly config: Required<Omit<ClientConfig, 'fetch'>> & { fetch: FetchLike };

  constructor(options: LeadWeaveClientOptions) {
    if (!options.baseUrl) throw new Error('LeadWeaveClient: baseUrl is required');
    if (!options.apiKey) throw new Error('LeadWeaveClient: apiKey is required');
...
export default LeadWeaveClient;
