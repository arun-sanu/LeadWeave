...
- **Client entry point** — `leadweave.New(baseURL, apiKey, opts...)` returns a
...
surfaces as `*leadweave.TimeoutError`.
...
tracing := func(next http.RoundTripper) http.RoundTripper {
	return leadweave.RoundTripperFunc(func(req *http.Request) (*http.Response, error) {
		// start span, inject headers…
		return next.RoundTrip(req)
	})
}
client, _ := leadweave.New(baseURL, apiKey, leadweave.WithMiddleware(tracing))
```
...
client, _ := leadweave.New("https://api.test", "key", leadweave.WithTransport(mockRT{}))
```
