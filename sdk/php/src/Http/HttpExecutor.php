...
class HttpExecutor
{
     ...
     *
     * @throws LeadWeaveApiException  On any non-2xx response (typed subclass).
     * @throws LeadWeaveTimeoutException On timeout.
     */
    ...
    private function send(string $method, string $path, array $query, $body): ResponseInterface
    {
        ...
        try {
            ...
            if ($isTimeout) {
                throw new LeadWeaveTimeoutException($this->timeout);
            }
            throw $e;
        }
    ...
    private function buildApiException(ResponseInterface $response, string $method, string $path): LeadWeaveApiException
    {
        $status = $response->getStatusCode();
        $text = (string) $response->getBody();
        $data = null;
        ...
        $message = "LeadWeave API {$status} {$reason} — {$method} {$path}: {$messageText}";

        return LeadWeaveApiException::classify($status, $message, $data, $envelope['error'] ?? null);
    }
}
