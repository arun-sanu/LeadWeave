...
```python
with LeadWeaveClient(base_url="…", api_key="…") as client:
...
```python
import httpx
client = LeadWeaveClient(base_url="…", api_key="…", transport=httpx.MockTransport(handler))
```
...
A non-2xx response raises a typed `LeadWeaveApiError` subclass — `LeadWeaveAuthError` (401),
`LeadWeaveForbiddenError` (403), `LeadWeaveNotFoundError` (404), `LeadWeaveConflictError` (409),
`LeadWeaveRateLimitError` (429), `LeadWeaveNotImplementedError` (501),
`LeadWeaveServiceUnavailableError` (503 — the only retryable one) — each carrying `.status`
and the parsed `.body`. A timeout raises `LeadWeaveTimeoutError`.

```python
from leadweave import LeadWeaveNotFoundError

try:
    client.sessions.get("missing")
except LeadWeaveNotFoundError as e:
    print(e.status)  # 404
```
