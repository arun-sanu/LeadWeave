"""HTTP transport wrapper for the LeadWeave Python SDK.
...
from .errors import LeadWeaveApiError, LeadWeaveTimeoutError, classify
...
class HttpExecutor:
    ...
    Constructed once per :class:`LeadWeaveClient`; the transport is taken from
    ...
    def _send(self, method: HttpMethod, path: str, *, query: Mapping[str, Any] | None, body: Any) -> httpx.Response:
        ...
        try:
            ...
            raise LeadWeaveTimeoutError(self._timeout) from e
        ...
        if res.status_code >= 300:
            ...
            raise LeadWeaveApiError.from_response(res.status_code, res.text, context)
        return res
