"""LeadWeave Python SDK — client core.

The :class:`LeadWeaveClient` is the single entry point. It owns an
...
def _warn_if_insecure_http(url: str) -> None:
    ...
    try:
        ...
        if parsed.scheme == "http" and host not in _LOCALHOST_HOSTS:
            warnings.warn(
                f"LeadWeaveClient: base_url uses an insecure http:// URL (host: {host}). "
...
class LeadWeaveClient:
    def __init__(
        ...
        *,
        ...
        if not base_url:
            raise ValueError("LeadWeaveClient: base_url is required")
        if not api_key:
            raise ValueError("LeadWeaveClient: api_key is required")
    ...
    def __enter__(self) -> "LeadWeaveClient":
        return self
