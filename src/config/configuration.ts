...
/**
 * The plugin package dir LeadWeave ≤ 0.12.1 defaulted to. A host that ran on that default has working
...
export default () => ({
  ...
  // LAN Mesh peer discovery (mDNS/Bonjour). Strictly opt-in via LAN_MESH_ENABLED=true for office LANs.
  lanMesh: {
    enabled: process.env.LAN_MESH_ENABLED === 'true',
  },
  ...
  dataDatabase: {
    ...
    // behavior; set POSTGRES_SCHEMA to place LeadWeave's tables + the TypeORM migration ledger in a
    // dedicated schema (e.g. a managed-Postgres project schema, or to isolate LeadWeave from other
  ...
  plugins: {
    ...
    catalogUrl:
      ...
      'https://raw.githubusercontent.com/arun-sanu/LeadWeave-plugins/main/plugins.json',
