...
describe('validateEnv', () => {
  ...
  it('validates POSTGRES_SCHEMA as a legal, non-reserved Postgres identifier when set', () => {
    ...
    expect(() => validateEnv({ ...pg, POSTGRES_SCHEMA: 'leadweave' })).not.toThrow();
    ...
    expect(() => validateEnv({ ...pg, POSTGRES_SCHEMA: 'leadweave; DROP' })).toThrow(/POSTGRES_SCHEMA/);
  ...
  it('rejects a sqlite data DB path that collides with the internal main database file', () => {
    ...
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: './data/leadweave.sqlite' })).not.toThrow();
  ...
  it('resolves the main DB path from MAIN_DATABASE_NAME like the runtime (no false-negative/-positive)', () => {
    ...
    expect(() =>
      validateEnv({
        ...
        MAIN_DATABASE_NAME: '/srv/leadweave/main.sqlite',
        DATABASE_NAME: '/srv/leadweave/main.sqlite',
      }),
  ...
  it('rejects a bare SQLite DATABASE_NAME (PG-name leak) that has no path separator or file extension', () => {
    // Regression for #677: .env.example shipped `DATABASE_NAME=leadweave` (a PostgreSQL db name).
    // In a SQLite run that bare name becomes the file PATH → SQLite opens a file named 'leadweave'
    // under the read-only app rootfs → SQLITE_CANTOPEN boot-loop. The guard catches the leak at boot.
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: 'leadweave' })).toThrow(/file path/);
    ...
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: 'leadweave.sqlite' })).not.toThrow();
    ...
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: '/app/data/leadweave.sqlite' })).not.toThrow();
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: './data/leadweave.sqlite' })).not.toThrow();
