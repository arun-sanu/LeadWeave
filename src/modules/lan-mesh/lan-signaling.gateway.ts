...
export class LanSignalingGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleJoinMesh(@MessageBody() data: { name: string }, @ConnectedSocket() client: Socket) {
  ...
  handleWebRtcSignal(@MessageBody() data: { targetPeerId: string; signal: any }, @ConnectedSocket() client: Socket) {
