...
The plugin architecture enables LeadWeave extensibility without modifying the core codebase. Plugins can add new features, integrate with external services, or customize behavior.
...
```json
{
  ...
  "version": "1.0.0",
  "type": "extension",
  "description": "An awesome plugin for LeadWeave",
  "author": "Your Name",
  "license": "MIT",
...
import type { IPlugin, PluginContext } from '@leadweave/plugin-sdk'; // shape only; implement IPlugin
...
> Plugins implement the `IPlugin` interface directly. An `@leadweave/plugin-sdk` npm package is planned
...
```mermaid
sequenceDiagram
    participant Core as LeadWeave Core
