...
# Check Docker logs (service name in the shipped production compose; it is `leadweave` in
# docker-compose.dev.yml)
docker compose logs leadweave-api

# Common fixes
docker system prune -f         # Clean up dangling images/containers
git pull                       # The shipped compose BUILDS leadweave-api from source —
docker compose up -d --build   # `docker compose pull` never updates it
```

> Do **not** reach for `docker compose down --volumes` here. It deletes the `leadweave-data` volume,
...
**Cause:** In production LeadWeave sends the CSP `upgrade-insecure-requests` directive, which tells the
...
docker compose exec leadweave-api printenv NODE_ENV CSP_UPGRADE_INSECURE_REQUESTS
```
...
> The alternative is to front LeadWeave with a TLS-terminating reverse proxy (the shipped
...
docker compose logs leadweave-api 2>&1 | grep -i "whatsapp\|puppeteer\|browser"
...
docker compose exec leadweave-api ls -la /app/data/sessions/session-<name>/   # whatsapp-web.js
docker compose exec leadweave-api ls -la /app/data/baileys/<name>/            # baileys
```
...
docker compose exec leadweave-api rm -rf /app/data/sessions/session-<name>   # whatsapp-web.js
docker compose exec leadweave-api rm -rf /app/data/baileys/<name>            # baileys
docker compose restart leadweave-api
```

> The service name above is the one in the shipped production `docker-compose.yml` (`leadweave-api`),
> which mounts `/app/data` from a **named volume** — there is no `./data` on the host to inspect;
> reach into the container (`docker compose exec leadweave-api ls /app/data/sessions`) instead. In
> `docker-compose.dev.yml` the app service is called `leadweave` and `./data` is bind-mounted, so the
...
**Fix:** LeadWeave reconciles a missed `ready` event when WhatsApp Web is connected, the injected
...
`WWEBJS_WEB_VERSION` unset, `latest`, or `auto` (the default), LeadWeave auto-resolves a settled build
...
`$HOME`**. The non-root `leadweave` user has no home dir, so Chromium tries to use `/home/leadweave`, which does
...
owned by `leadweave`). If you run a custom container, ensure both are set to a writable, existing path:
...
```bash
docker stats leadweave-api   # watch the PIDS column — does it climb toward the limit right before the failure?
```

_Fix:_ raise the ceiling. The bundled `docker-compose.yml` exposes it as `LEADWEAVE_PIDS_LIMIT` (default `2048`,
which fits ~8-10 sessions with startup-spike headroom):

```bash
LEADWEAVE_PIDS_LIMIT=4096   # in your .env, then docker compose up -d
```
...
# Docker Desktop: check the VM via the app, or nudge LEADWEAVE_MEM_LIMIT up and retry
```

_Fix:_ raise the ceiling (`LEADWEAVE_MEM_LIMIT=4g` in your `.env`, or Docker Desktop → Settings → Resources →
Memory for the VM).
...
**Quick triage:** run `docker stats leadweave-api`, click **Start**, and watch which resource spikes toward its
...
**Start** after upgrading LeadWeave — no QR is produced — and the session's `lastError` / container log
show:
...
```bash
docker exec leadweave-api rm -rf /app/data/sessions/session-<name>
...
LeadWeave returns to a fresh QR because `whatsapp-web.js` deletes LocalAuth credentials on `LOGOUT`.

**Cause:** LeadWeave used to backfill active WhatsApp Status posts immediately in its `ready` callback by
...
> heading ("What's new"). The language WhatsApp Web renders in follows the browser locale, which LeadWeave
...
reads. LeadWeave ships a backport that restores it, applied at install time by
...
docker compose logs leadweave-api 2>&1 | grep -i webhook
...
```bash
# Check memory usage
docker stats leadweave-api --no-stream

# Check process memory (Prometheus text; read leadweave_process_resident_memory_bytes)
...
services:
  leadweave-api:
    # The shipped compose already exposes this as mem_limit: ${LEADWEAVE_MEM_LIMIT:-2g}
    mem_limit: 2g
    environment:
...
Pooling and caching are environment variables — LeadWeave has no config file:
...
sqlite3 ./data/leadweave.sqlite ".timeout 30000"

# Check WAL mode
sqlite3 ./data/leadweave.sqlite "PRAGMA journal_mode;"
# Default is: delete (rollback journal) — LeadWeave does not force WAL

# Optionally enable WAL mode to reduce writer/reader lock contention
sqlite3 ./data/leadweave.sqlite "PRAGMA journal_mode=WAL;"
```
...
docker exec leadweave-api ping postgres
docker exec leadweave-api curl http://host.docker.internal:8080
```
...
**Q: Is LeadWeave safe to use?**

> A: LeadWeave uses unofficial WhatsApp Web API. While we implement best practices to avoid detection, there's inherent risk of account restrictions. We recommend:
>
...
> A: Yes, LeadWeave works with both personal and WhatsApp Business accounts. Note that WhatsApp Business API (official Meta API) is different and not supported.
...
> 2. Set URL: `http://leadweave:2785/api/sessions/{id}/messages/send-text`
...
**Then set `TRUSTED_PROXIES` in your `.env`.** With it empty, LeadWeave correctly refuses to trust the
...
`127.0.0.1`. LeadWeave logs a one-time warning at the first proxied request when the header is present
but the list is empty.

**Q: How to run behind Traefik / Coolify?**

Traefik forwards WebSocket upgrades automatically, so LeadWeave's single-port Socket.IO channel works with a normal HTTP router. Two things keep a public deployment stable:

**1. Let Traefik reach the container over the Docker network — don't _also_ publish the host port.** This is the most common cause of intermittent `504`s on Coolify/Traefik. If LeadWeave publishes its port to the host (`ports: ["2785:2785"]`) **and** Traefik also routes to it, every request additionally traverses Docker's userland `docker-proxy`. LeadWeave holds a long-lived Socket.IO connection per client (HTTP long-poll → WebSocket upgrade), so those held-open connections accumulate across both hops and gradually exhaust the connection pool to the single upstream — the Dashboard, API, and real-time channel then `504` together "after some time", while `curl http://localhost:2785/api/health/ready` keeps returning `200`. Front it with Traefik on a shared network and **expose** the port internally instead of **publishing** it:
...
services:
  leadweave:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.leadweave.rule=Host(`api.example.com`)
      - traefik.http.routers.leadweave.entrypoints=websecure
      - traefik.http.routers.leadweave.tls.certresolver=le
      - traefik.http.services.leadweave.loadbalancer.server.port=2785
networks:
  proxy:
...
Remember LeadWeave is **single-port**: the Dashboard, REST API, and Socket.IO all share `:2785` behind one router, so a choked upstream takes all three down at once. A Dashboard stuck on "Connecting…" while `localhost` is healthy is the proxy hop, not the app.

**Q: How to backup sessions automatically?**

```bash
# Add to crontab, for example: 0 */6 * * * cd /path/to/leadweave && ./scripts/backup.sh
BACKUP_DIR=/backups/leadweave ./scripts/backup.sh
```
...
2. **Check logs** - `docker compose logs leadweave-api --tail=100`
