...
curl -X POST "$BASE/api/sessions/$SESSION_ID/messages/send-text" \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ "chatId": "628123456789@c.us", "text": "Hello from LeadWeave!" }'
```
...
curl -X POST "$BASE/api/sessions/$SESSION_ID/status/send-text" \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ "text": "Hello from LeadWeave!", "backgroundColor": "#25D366", "font": 2 }'
```
...
curl -X PUT "$BASE/api/infra/config" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "database": { "type": "postgres", "host": "db.example.com", "port": "5432", "username": "leadweave", "password": "s3cret", "database": "leadweave", "poolSize": 10, "sslEnabled": true, "sslRejectUnauthorized": false },
...
curl -X POST "$BASE/api/plugins/install-url" \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://github.com/leadweave-plugins/chat-flow/releases/download/v1.0.0/chat-flow.zip" }'
```
...
curl -X POST "$BASE/mcp" \
  -H "X-Api-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ "jsonrpc": "2.0", "id": 1, "method": "initialize", "params": { "protocolVersion": "2025-06-18", "capabilities": {}, "clientInfo": { "name": "leadweave-collection", "version": "1.0.0" } } }'
